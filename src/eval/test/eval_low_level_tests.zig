...
pub const tests = [_]TestCase{
    ...
    // is past the I64 range still converts. Narrowing through an i64 on the way
    // would lose the high bits or fail outright.
    .{
    ...
    // A negative Dec converted to an unsigned width. The result's high bits come
    // from the sign, so a conversion that widens a narrower quotient has to carry
    // that sign rather than pad with zeroes.
    .{
    ...
    // them. These share a lowering path with the integer-to-integer conversions,
    // so narrowing that path has to leave them working.
    .{
