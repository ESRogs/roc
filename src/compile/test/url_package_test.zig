...
test "issue 9509: transitive URL dependency compiles through the orchestration core from a warm cache" {
    ...
    const diag = try build_env.renderDiagnosticsWithConfig(null, @import("reporting").ReportingConfig.initColorTerminal());
    try std.testing.expectEqual(@as(usize, 0), diag.errors);
    try std.testing.expectEqual(@as(usize, 0), diag.warnings);

    // Executable finalization succeeded, so a run back half may lower this.
    try std.testing.expect(build_env.executableLoweringReady());
