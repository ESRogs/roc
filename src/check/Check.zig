...
fn exactNumeralInfoForExpr(self: *const Self, expr_idx: CIR.Expr.Idx, region: Region) Allocator.Error!types_mod.NumeralInfo {
    ...
    const fit_set = try exact_numeral.computeFitSet(self.gpa, exact);
    return types_mod.NumeralInfo.fromExact(exact, fit_set, region);
}
...
fn checkDef(self: *Self, def_idx: CIR.Def.Idx, env: *Env) std.mem.Allocator.Error!void {
    ...
    const def_is_function = isFunctionDef(&self.cir.store, self.cir.store.getExpr(def.expr));
                ...
                while (constraint_i < constraints_len) : (constraint_i += 1) {
                    ...
                    if (constraint.fn_name.eql(self.cir.idents.encoder_for)) {
                        ...
                        if (supports_implicit_encode == .unsupported) {
                            if (self.nominalListPayloadVar(nominal_type)) |payload_var| {
                                supports_implicit_encode = try self.varSupportsDerivedEncodeShape(payload_var, encoding_var, env, region);
                            }
                        }
                        if (supports_implicit_encode == .unsupported) {
                            if (self.nominalBoxPayloadVar(nominal_type)) |payload_var| {
...
fn validateBuiltinFromNumeralLiteral(
    ...
    return switch (target) {
        // Floats represent every literal (out-of-range becomes ±inf), so they
        // are in every fit set and never reach this branch.
        .f32, .f64 => null,
