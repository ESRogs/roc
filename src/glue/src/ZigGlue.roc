...
zig_record_field_decl = |type_table, duplicate_tag_names, preferred_names, field, use_32| {
...
zig_record_fields_decl = |type_table, duplicate_tag_names, preferred_names, fields, use_32| {
	var $field_strs = ""
	for field in fields {
		$field_strs = Str.concat($field_strs, zig_record_field_decl(type_table, duplicate_tag_names, preferred_names, field, use_32))
	}
	$field_strs
}
			...
			RocRecord(rec) =>
				if rec.name != "" {
					...
					if !(List.contains($seen_names, struct_name)) {
						...
						native_field_strs = zig_record_fields_decl(type_table, duplicate_tag_names, preferred_names, rec.fields, Bool.False)
						wasm32_fields = rec.fields
						wasm32_field_strs = zig_record_fields_decl(type_table, duplicate_tag_names, preferred_names, wasm32_fields, Bool.True)
	...
	for func in hosted_functions {
		...
		if type_table_result.found {
			...
			for field in type_table_result.fields {
				$fields_64 = Str.concat($fields_64, zig_record_field_decl(type_table, duplicate_tag_names, preferred_names, field, Bool.False))
				$fields_32 = Str.concat($fields_32, zig_record_field_decl(type_table, duplicate_tag_names, preferred_names, field, Bool.True))
			}
