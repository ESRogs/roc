...
- The authoritative Rocci Bird `--opt=size --target=wasm32` cart is 30,946
  bytes with idiomatic iterators.
- The equivalent direct-list cart is 30,877 bytes.
- The iterator premium is 69 bytes.
...
The remaining 20,291-byte gap to the 10,655-byte Rust cart is general runtime,
...
7. **A second iterator-only fusion representation.** The implemented general
   ...
   cart premium is only 69 bytes. More iterator-specific machinery requires a
...
After the later merge from main and the narrowed value-aware specialization
gate, fresh cleared-cache measurements were 30,946 bytes for the iterator cart
and 30,877 bytes for the direct-list cart. The iterator premium is now 69
bytes.
...
without materially changing the current 69-byte iterator premium. Further work toward
