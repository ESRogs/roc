...
`LirCodeGen` is the sole authority for semantic value locations. Every assigned
...
generators own only physical register masks for short-lived
...
or reload semantic values independently.
...
one newly allocated physical register per recursive layout, control-flow node,
